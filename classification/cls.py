...
class Vanilla_NN(Base):
    ...
    def _logpred(self, x, y, task_idx):
        logits = self(x, task_idx)
        targets = torch.argmax(y, dim=1)
        return F.cross_entropy(logits, targets, reduction="mean")

    def prediction_prob(self, x_test, task_idx):
        with torch.no_grad():
            outputs = self(torch.from_numpy(x_test).to(device), task_idx)
...
class MFVI_NN(Base):
    ...
    def _logpred(self, x, y, task_idx):
        logits = self(x, task_idx, self.no_train_samples)
        K, bs, num_cls = logits.size()
        target = y.argmax(dim=1)
        target = target.unsqueeze(0).expand(K, bs)
        logits_flat = logits.view(-1, num_cls)
    ...
    def prediction_prob(self, x_test, task_idx):
        with torch.no_grad():
            ...
            return (
                F.softmax(outputs, dim=2).cpu().numpy()
            )  # (no_pred_samples, len(x_test), no_classes)
