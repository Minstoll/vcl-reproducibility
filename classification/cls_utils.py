...
from cls import Vanilla_NN, MFVI_NN
...
def uncertainty_coreset(
    ...
    if isinstance(model, Vanilla_NN):
        return rand_from_batch(
            x_coreset, y_coreset, x_train, y_train, coreset_size
        )  # If vanilla, nothing to avg over, so equal mutual info rankings. use rand
    with torch.no_grad():
        preds = model(
            torch.FloatTensor(x_train).to(device),
            task_id - 1,
            model.no_train_samples,  # need prev id since using prev iteration mf_model
        )
    pred_means = preds.mean(dim=0)
    pred_entropy = torch.distributions.Categorical(
        logits=pred_means
    ).entropy()  # shape [N]
    ...
    idx = idx.cpu().numpy()
    idx = idx[::multiplier]
    # idx = np.random.choice(idx, coreset_size, replace=False)
...
# if setting unc_coreset=True, automatically overrids coreset_method with uncertainty_coreset.
def run_vcl(
    hidden_size,
    no_epochs,
    data_gen,
    ...
    for task_id in range(data_gen.max_iter):
        ...
        # initial model for point estimate
        if task_id == 0:
            ml_model = Vanilla_NN(
                in_dim,
                hidden_size,
            ...
            if unc_coreset:
                if task_id != 0:
                    x_coresets, y_coresets, x_train, y_train = uncertainty_coreset(
                        ...
                        y_train.cpu().numpy(),
                        coreset_size,
                        mf_model,  # mf_model here from prev task
                        head,
                    )
            else:
